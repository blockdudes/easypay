...
export const fetchOnBoardingData = createAsyncThunk("fetchOnBoardingData", async ({ signer }: { signer: string }, { rejectWithValue }) => {
    try {
        const data = (await axios.get(`http://localhost:3000/get-asset-receiver?owner=${signer}`)).data;
        return { address: data?.address, createAt: new Date, chainId: data?.chainId, token: data?.token, owner: data?.owner }
    } catch (error) {
